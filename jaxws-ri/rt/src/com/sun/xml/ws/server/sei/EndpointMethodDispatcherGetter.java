...
public class EndpointMethodDispatcherGetter {
    ...
    public EndpointMethodDispatcherGetter(SEIModel model, WSBinding binding, InvokerSource invokerTube, boolean isResponseProcessor, EndpointMethodHandlerFactory factory) {
        dispatcherList = new ArrayList<EndpointMethodDispatcher>();

        if (binding.getAddressingVersion() != null) {
            dispatcherList.add(
            		isResponseProcessor ? 
            				new ResponseActionBasedDispatcher(model, binding, invokerTube, this, factory) 
            		: new ActionBasedDispatcher(model, binding, invokerTube, this, factory));
        }
        ...
        dispatcherList.add(new PayloadQNameBasedDispatcher(model, binding, invokerTube, isResponseProcessor, this, factory));
        dispatcherList.add(new SOAPActionBasedDispatcher(model, binding, invokerTube, isResponseProcessor, this, factory));
    }
    ...
    public Method getHandlerMethod(JavaMethodImpl jmi) {
