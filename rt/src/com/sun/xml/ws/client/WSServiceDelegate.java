...
public class WSServiceDelegate extends WSService {
   ...
   private final Map<QName,SEIPortInfo> seiContext = new HashMap<QName,SEIPortInfo>();
    ...
    public void setPortEpr(QName portName, WSEndpointReference epr) {
        PortInfo portInfo = new PortInfo(this, (epr.getAddress() == null) ? null : EndpointAddress.create(epr.getAddress()), portName,
                getPortModel(wsdlService, portName).getBinding().getBindingId());
        ports.put(portName, portInfo);
    }
    ...
    private <T> T createEndpointIFBaseProxy(@Nullable WSEndpointReference epr,QName portName, Class<T> portInterface,
        ...
        ClassLoader loader = 
          new DelegatingLoader(portInterface.getClassLoader(),
    ...
    protected InvocationHandler getStubHandler(BindingImpl binding, SEIPortInfo eif, @Nullable WSEndpointReference epr) {
    	...
    	return new SEIStub(eif, binding, eif.model, eif.sei, epr);
    }
